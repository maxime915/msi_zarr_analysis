...
def interpret_forest_ds(
    ...
    # save_ds_heatmap(dataset, None, None)

    # # export tree
    # # from sklearn import tree
    # # print(tree.export_text(forest))
    # # print(f"{dataset.attribute_names()[73]=}")

    # # try new method
    # for depth in range(1, 10):
    #     dataset_x_ = np.copy(dataset_x)
    #     X_train_ = np.copy(X_train)

    #     n_removed = 0

    #     cv_scores = 2 * imbalance
    #     while np.min(cv_scores) > 1.5 * imbalance:
    #         model = DecisionTreeClassifier(max_depth=depth)
    #         cv_scores = evaluate_cv(model, dataset_x_, dataset_y)
    #         model.fit(X_train_, Y_train)
    #         idx = np.argmax(model.feature_importances_)

    #         print(
    #             f"{depth=:d} {n_removed=} {idx=:02d} {dataset.attribute_names()[idx]=} {np.min(cv_scores)=:.3f}"
    #         )
...
def save_as_blob(
    ...
    fig, ax = plt.subplots(1, 1, figsize=(70, 50))
    plt.axis("off")
    ...
    ax.matshow(np.concatenate([ds_x[mask], ds_x[~mask]]), aspect="auto")
